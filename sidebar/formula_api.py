...
import google.generativeai as stable_genai

def show_formula_api():
    st.title("📡 Formula API")
    st.subheader("Get Exam-Ready Formulas for Last-Minute Revisions!")

    query = st.text_input("Enter Topic Path", placeholder="e.g., /physics/thermodynamics", key="api_query_input")
    ...
    if st.button("Execute Request"):
        if not query or len(query) < 3:
            st.warning("Please enter a valid topic path.")
            return

        if "GEMINI_API_KEY" not in st.secrets:
            st.error("API Key missing in st.secrets")
            return
            
        try:
            # 1. Setup
            api_key = st.secrets["GEMINI_API_KEY"].strip()
            stable_genai.configure(api_key=api_key)
            
            # 2. Use the stable model name
            model = stable_genai.GenerativeModel('gemini-1.5-flash')
            ...
            with st.spinner("Fetching from QSense API..."):
                # 3. Call the stable generate method
